...
import (
	...
	_oauth2Model "github.com/jakkaphatminthana/isekai-shop-api/pkg/oauth2/model"
...
func (c *googleOAuth2Controller) PlayerLoginCallback(pctx echo.Context) error {
	...
	//get token (https://.../callback?code=abcde) -> {accessToken, refreshToken}
...
func (c *googleOAuth2Controller) setCookie(pctx echo.Context, name string, value string) {
	cookie := &http.Cookie{
		Name:     name,
		Value:    value,
		Path:     "/",
		HttpOnly: true, //unable read on javascript
		//TODO : turn it on when deploy prodution
		// Secure: true,
	}
...
// validate https://.../callback?code=abcde
...
func (c *googleOAuth2Controller) setSameSiteCookie(pctx echo.Context, name string, value string) {
	cookie := &http.Cookie{
		Name:     name,
		Value:    value,
		Path:     "/",
		HttpOnly: true,                    //unable read on javascript
		SameSite: http.SameSiteStrictMode, //avoid sending cookie cross website
		//TODO : turn it on when deploy prodution
		// Secure: true,
	}
	pctx.SetCookie(cookie)
